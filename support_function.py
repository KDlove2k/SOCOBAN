...
class state:
    def __init__(self, board, state_parent, list_check_point):
        ...
        self.box_weights = self.init_box_weights()  
    # Trọng lượng viên đá
    def init_box_weights(self, rock_weights):
        weights = {}
        if rock_weights:
            with open(rock_weights, 'r') as f:
                for line in f:
                    weight = list(map(int, line[0].strip().split(',')))
        else:
            for i in range(len(self.board)):
                for j in range(len(self.board[i])):
                    if self.board[i][j] == '$':
                        weights[(i, j)] = 1  # Gán trọng lượng mặc định
        return weights
...
def move(board, next_pos, cur_pos, list_check_point):
    ...
    if new_board[next_pos[0]][next_pos[1]] == '$':
        ...
        new_board[x][y] = '$'
    weight = new_board[next_pos[0]][next_pos[1]].get('weight', 1)  # Lấy trọng lượng
    new_board[x][y]['weight'] += weight  # Cộng trọng lượng viên đá
